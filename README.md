# CNU Library AI Agent (BUA)

충남대학교 도서관 AI Agent - Browser Use Agent 기반 브라우저 자동화

## 📋 개요

Samsung Research의 "Browser Use Agent 개발 여정" (김기훈)을 참고하여 구현한 LLM 기반 브라우저 자동화 에이전트입니다.

> ⚠️ **개발 중**: 현재 개발 중인 프로젝트로 일부 기능이 불안정하거나 작동하지 않을 수 있습니다.

| 항목 | 설명 |
|------|------|
| **방식** | DOM Snapshot + LLM 판단 (ReAct 패턴) |
| **브라우저** | Playwright (headless=False로 실행 과정 확인 가능) |
| **LLM** | OpenAI GPT API |

---

## 📁 프로젝트 구조

```
mcp-server/
├── bua/                          # Browser Use Agent 모듈
│   ├── __init__.py
│   ├── snapshot.py               # DOM 스냅샷 추출
│   ├── tools.py                  # 브라우저 액션 (click, type, navigate...)
│   └── agent.py                  # ReAct 루프 + LLM 판단
│
├── bua_server.py                 # BUA 서버 (CLI + MCP)
├── book_crawler.py               # 기존 RPA 방식 크롤러
├── server.py                     # 기존 MCP 서버 (시설 정보)
├── http_server.py                # HTTP API (크롬 확장 연동)
├── requirements.txt
└── claude_desktop_config.json
```

---

## 🔄 버전 비교

| 구분 | book_crawler.py | bua/ |
|------|-----------------|------|
| **방식** | RPA (하드코딩 셀렉터) | AI Agent (LLM 판단) |
| **LLM 필요** | ❌ | ✅ OpenAI API |
| **범용성** | 충남대 도서관 전용 | 어떤 사이트든 가능 |
| **유지보수** | 사이트 변경 시 코드 수정 | 프롬프트만 수정 |

---

## 🚀 설치 및 실행

### 1. 가상환경 설정

```powershell
cd C:\cnu-library-rag-3\mcp-server
python -m venv venv
.\venv\Scripts\Activate
```

### 2. 의존성 설치

```powershell
pip install -r requirements.txt
playwright install chromium
```

### 3. API 키 설정

```powershell
$env:OPENAI_API_KEY = "sk-proj-..."
```

### 4. CLI 실행

```powershell
python bua_server.py
```

---

## 🎮 CLI 사용법

```
============================================================
BUA (Browser Use Agent) - CLI Mode
============================================================

Commands:
  1. Agent Run (natural language goal)   # LLM 자동 판단
  2. Navigate to URL                     # URL 이동
  3. Page Snapshot                       # 페이지 분석
  4. Click Element                       # 요소 클릭
  5. Type Text                           # 텍스트 입력
  6. Press Key                           # 키 입력
  s. Save results to JSON                # 결과 저장
  q. Quit                                # 종료 (자동 저장)

Select>
```

### 예시: 로그인

```
Select> 1
Goal: 아이디는 202302554이고, 비밀번호는 mypassword야. 로그인해줘.
Start URL: https://library.cnu.ac.kr/
```

**실행 과정:**
```
Step 1: 팝업 닫기 (a.infoClose 클릭)
Step 2: 로그인 버튼 클릭 (a[href='/login'])
Step 3: 학번 입력 (input#id)
Step 4: 비밀번호 입력 (input[name='password'])
Step 5: Enter 키
Step 6: "로그아웃" 텍스트 확인 → done
```

### 예시: 책 검색

```
Select> 1
Goal: 자바의 정석 검색해줘
Start URL: https://library.cnu.ac.kr/
```

---

## 🔧 Claude Desktop 연동 (MCP)

> ⚠️ **주의**: MCP 서버 기능은 현재 수정 중입니다.

`%APPDATA%\Claude\claude_desktop_config.json`:

```json
{
  "mcpServers": {
    "cnu-library-bua": {
      "command": "python",
      "args": ["C:\\cnu-library-rag-3\\mcp-server\\bua_server.py", "--mcp"],
      "env": {
        "OPENAI_API_KEY": "sk-proj-..."
      }
    }
  }
}
```

### BUA MCP Tools (수정 중)

| Tool | 설명 |
|------|------|
| `browser_agent_run` | 자연어 목표로 자동화 (LLM 판단) |
| `browser_navigate` | URL 이동 |
| `browser_snapshot` | 페이지 DOM 분석 |
| `browser_click` | 요소 클릭 |
| `browser_type` | 텍스트 입력 |
| `browser_screenshot` | 스크린샷 저장 |

---

## 🎯 아키텍처

```
┌─────────────────────────────────────────────────────────────┐
│                        사용자                                │
│              "로그인해줘" / "자바의 정석 검색해줘"              │
└─────────────────────────┬───────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│                    bua_server.py                            │
│                   (CLI / MCP 서버)                           │
└─────────────────────────┬───────────────────────────────────┘
                          │
          ┌───────────────┼───────────────┐
          ▼               ▼               ▼
   ┌────────────┐  ┌────────────┐  ┌────────────┐
   │ snapshot.py│  │  tools.py  │  │  agent.py  │
   │ DOM 추출   │  │ 브라우저   │  │ ReAct 루프 │
   │            │  │ 액션       │  │ LLM 판단   │
   └────────────┘  └────────────┘  └────────────┘
          │               │               │
          └───────────────┼───────────────┘
                          │
          ┌───────────────┴───────────────┐
          ▼                               ▼
   ┌────────────┐                  ┌────────────┐
   │ Playwright │                  │   OpenAI   │
   │ 브라우저   │                  │  GPT API   │
   └────────────┘                  └────────────┘
```

---

## 📊 ReAct 루프

```
1. DOM Snapshot 추출
   └─ 클릭/입력 가능 요소, 셀렉터 정보
          │
          ▼
2. LLM Prompt 생성
   └─ 목표 + 페이지상태 + 히스토리 + Site-specific 규칙
          │
          ▼
3. LLM 액션 결정
   └─ {"action": "click", "selector": "#login-btn"}
          │
          ▼
4. Playwright 실행
   └─ 실제 브라우저에서 클릭/입력
          │
          ▼
5. 결과 관찰 → 목표 달성? → Yes: done / No: 1번으로
```

---

## 📝 JSON 결과 예시

종료 시 자동 저장: `bua_result_20241230_145530.json`

```json
{
  "session_start": "2024-12-30T14:50:08.123456",
  "session_end": "2024-12-30T14:55:30.654321",
  "actions": [
    {
      "type": "navigate",
      "url": "https://library.cnu.ac.kr",
      "result": {"success": true, "message": "Navigation completed"},
      "timestamp": "2024-12-30T14:50:15.123456"
    },
    {
      "type": "click",
      "selector": "a.infoClose",
      "result": {"success": true, "message": "Click completed"},
      "timestamp": "2024-12-30T14:50:18.123456"
    },
    {
      "type": "type",
      "selector": "#id",
      "text": "202302554",
      "result": {"success": true, "message": "Input completed"},
      "timestamp": "2024-12-30T14:50:25.123456"
    }
  ]
}
```

---

## ⚠️ 주의사항

| 항목 | 설명 |
|------|------|
| **API 비용** | 매 Step마다 OpenAI API 호출 |
| **속도** | LLM 응답 대기로 RPA보다 느림 |
| **API 키 필수** | `browser_agent_run` 사용 시 필요 |
| **브라우저 표시** | headless=False로 실행 과정 확인 가능 |

---

## 📋 구현 상태 (PDF 기준)

| 개념 | 상태 | 설명 |
|------|------|------|
| Snapshot (DOM) | ✅ | 텍스트 기반 페이지 분석 |
| Screenshot (이미지) | ❌ | Vision 모델 활용 (미구현) |
| 하이브리드 | ❌ | Snapshot + Screenshot (미구현) |
| ReAct 루프 | ✅ | Thought → Action → Observation |
| Reflection | ✅ | 실패 시 원인 분석 |
| Post-summary | ✅ | 페이지 전환 시 요약 |
| Site-specific | ✅ | 충남대 도서관 로그인/검색 프롬프트 |

---

## 📋 TODO

- [ ] Screenshot 방식 추가 (Vision 모델)
- [ ] 하이브리드 전략 구현
- [ ] 분관대출 신청 자동화
- [ ] 도서 예약 자동화

---

## 🛠️ 개발

```powershell
# 의존성 설치
pip install playwright openai mcp

# 브라우저 설치
playwright install chromium

# CLI 실행
python bua_server.py

# MCP 모드 실행
python bua_server.py --mcp
```